            ...
            for i in idxs:
                ...
                if items:
                    ...
                    for j in items.get((k1, k2, k3, k4), []):
                        ...
                        o3[j[2]][j[4]] = 1
                        o4[j[3] - 1][j[4]] = 1
                    ok1, ok2, ok3, ok4, nk = np.array(items.get((k1, k2, k3, k4), [(0, 0, 0, 0, 0)])).T
                    ok1 = choice(ok1)
...
def extract_items(text_in):
    ...
    _k1, _k2 = np.where(_k1[0] > 0.8)[0], np.where(_k2[0] > 0.7)[0]
    _subjects = []
    for i in _k1:
        j = _k2[_k2 >= i]
        if len(j) > 0:
    ...
    if _subjects:
        ...
        for i, subject2 in enumerate(_subjects):
            _kk3, _kk4 = np.where(_k3[i] > 0.8)[0], np.where(_k4[i] > 0.7)[0]
            for _kkk3 in _kk3:
                for _kkk4 in _kk4:
                    if _kkk3 <= _kkk4:
        ...
        if _subject2s:
            ...
            for si, ssubject in enumerate(_subject2s):
                _oo1, _oo2 = np.where(_o1[si] > 0.8), np.where(_o2[si] > 0.7)
                for _ooo1, _c1 in zip(*_oo1):
                    for _ooo2, _c2 in zip(*_oo2):
                        if _ooo1 <= _ooo2 and _c1 == _c2:
                            _object = text_in[_ooo1 - 1: _ooo2]
                            _predicate = id2predicate[_c2]
                            _objects.append((ssubject[0],ssubject[1], ssubject[2], ssubject[3], ssubject[4], ssubject[5], _predicate, _object, _ooo1, _ooo2))
            ...
            if _objects:
                ...
                for n, oobject in enumerate(_objects):
                    _oo3, _oo4 = np.where(_o3[n] > 0.8), np.where(_o4[n] > 0.7)
...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f1 > self.best:
            self.best = f1
            train_model.save_weights('./data/final/best_model.weights')
...
if __name__ == '__main__':
    ...
    train_model.load_weights('./data/final/best_model.weights')
